...
exports.run = async (client, message, args) => {
  ...
  for (let record of finalContent) {
    ...
    foundRecordsArray.push(
      `***${i++}***. 📌 ${record.title} \n > \`\`Date\`\` : ${
        record.date
      } \n > \`\`Channel\`\` : ${
        record.channel
      } \n > \`\`Content\`\` : ${splicedRecordContent}..[read more](${
        record.link
      }) \n - \n`
    );
  }
  ...
  FieldsEmbedTagged.embed
    ...
    .setColor(0xff00ae)
    .setThumbnail(
      `https://media.discordapp.net/attachments/748005515992498297/756198492468281404/edit.png`
    )
  ...
  textRecordMessage.setDescription(
  ...
  let textRecordNumberCollector = await textRecordNumber.channel.awaitMessages(
    msgFilter,
    {
      max: 1,
      time: 300000,
  ...
  textRecordObject = finalContent[collectedNum - 1];
  textRecordMessageAfterConfirmation(message, textRecordObject, foundRecords);
};
